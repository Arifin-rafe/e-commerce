      ...
      if (guestCart.products.length === 0) {
        ...
        if (userCart) {
            guestCart.products.forEach((guestItem) => {
                ...
                if (productIndex > -1) {
                    // if product already exists in user cart, update the quantity
            ...
            // if the user has no cart, assign the guest cart to the user
        ...
        if(userCart){
            // guest cart already merged or does not exist, return the user cart
            res.status(200).json(userCart);
        }
